...
def main(argv):
    ...
    for path in data_list:
        model_save_dir_db = orig_model_save_dir
        if "old" in path:
            continue
        model_save_dir_db = model_save_dir_db.replace("XdB", path)
        print("Starting evaluation of dataset " + path)

        for i_c in range(1, 10):
            model_save_dir_corr = model_save_dir_db
            ctcr = float(i_c) / 10.0
            model_save_dir_corr = model_save_dir_corr.replace("YC",str(ctcr))
            config["cR"] = ctcr
            config["cT"] = ctcr
            ...
            if os.path.exists(os.path.join(model_save_dir_corr, "logs")):
                if remove_old:
                    shutil.rmtree(os.path.join(model_save_dir_corr, "logs"))
                    print("Deleted " + os.path.join(model_save_dir_corr, "logs"))
                else:
                    print("Skipped " + str(ctcr) + " " + path + " " + str(ctcr))
                    continue
            ...
            summaries = Summaries(scalar_summary_names=scalar_summary_names,
                                  ...
                                  save_dir=os.path.join(model_save_dir_corr),
                                  modes=["test"],
