...
def get_best_of_scalar(data_dict, name):
    ...
    if "f1_score" in name:
        ...
        # is_best_test = [True if test_idx in best_idx else False for test_idx in best_idx_test]
        best_idx = best_idx[-1]
...
def plot_over_snr(model_results_dict,gen_results_dict,corr,metric,models= ["self_attention_model",
                                                                           "mmse","oampnet","oampnet2"]):
    ...
    for model in model_results_dict:
        ...
        for snr in snr_vals:
            ...
            data_gen = gen_results_dict[model][snr][corr]
            db = int(snr.split("dB")[0])

            y_gen = data_gen
            y_train = data_trained
            if data_gen == 0 or not(np.isfinite(data_trained)):
                y = 1.0
            if model in all_results.keys():
                all_results[model].append(y_gen)
                all_results_train[model].append(y_train)
                all_db[model].append(db)
            else:
                all_results[model] = [y_gen]
                all_db[model] = [db]
    ...
    for model in models: 
        ...
        if model != "mmse":
            plt.plot(x, y, marker='o', linestyle='None',fillstyle="none")
            legend.append(model)
...
def plot_over_corr(model_results_dict,gen_results_dict,snr,metric,models= ["self_attention_model",
                                                                           "mmse","oampnet","oampnet2"]):
    ...
    for model in model_results_dict:
        ...
        for corr in corrs:
            if not(corr in gen_results_dict[model][snr].keys() and corr in model_results_dict[model][snr].keys()):
                continue
            data_trained = model_results_dict[model][snr][corr]
            data_gen = gen_results_dict[model][snr][corr]
            corr = float(corr)
...
def main(argv):
    ...
    gen_results_dict = get_gen_results(model_dir,metric,models)
    plot_over_snr(model_results_dict,gen_results_dict,corr,metric,models)
    plot_over_corr(model_results_dict,gen_results_dict,snr,metric,models)
